...
# --------------------------------------------------------
#
# Unchanged from the original Focal_Net.py (only relocated into the package),
# except that this file's own docstring/comments were added. `Signals_FocalNet`
# (dual-branch: horizontal + vertical patching, Section III-B/D of the paper)
# is the class this project actually uses; the generic `FocalNet` class and
# `focalnet_*` builder functions below it are the unmodified upstream
# single-branch image classifier this project's model was adapted from, kept
# for reference. See README.md > "Repository notes".
...
class PatchEmbed(nn.Module):
    ...
    def flops(self):
        ...
        return flops
############################################################################################################
class Signal_PatchEmbed(nn.Module):
    r""" Image to Patch Embedding
    ...
    def __init__(self, img_size=(224, 224), patch_sizew=4,patch_sizeh=3, in_chans=3, embed_dim=96, use_conv_embed=False,
                 norm_layer=None, is_stem=False):
        super().__init__()
        #patch_size = to_2tuple(patch_size)
        patches_resolution = [img_size[0] // patch_sizeh, img_size[1] // patch_sizew]
        self.img_size = img_size
        self.patch_size = (patch_sizeh, patch_sizew)
        self.patches_resolution = patches_resolution
        self.num_patches = patches_resolution[0] * patches_resolution[1]
    ...
    def flops(self):
        ...
        if self.norm is not None:
            flops += Ho * Wo * self.embed_dim
        return flops
class Signals_FocalNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 focal_levels_h=[2, 2, 2, 2],
                 focal_levels_v=[2, 2, 2, 2],
                 focal_windows_h=[3, 3, 3, 3],
                 focal_windows_v=[3, 3, 3, 3],
                 use_conv_embed=False,
        ...
        self.num_layers_h = len(depths_h)
        self.num_layers_v = len(depths_v)
        embed_dim_v = [embed_dim * (2 ** i) for i in range(self.num_layers_v)]
        embed_dim_h = [embed_dim * (2 ** i) for i in range(self.num_layers_h)]

        self.num_classes = num_classes
        self.embed_dim = embed_dim
        self.patch_norm = patch_norm
        self.num_features_v = embed_dim_v[-1]
        self.num_features_h = embed_dim_h[-1]
        self.mlp_ratio = mlp_ratio
        self.col_or_row = col_or_row

        # split image into patches using either non-overlapped embedding or overlapped embedding
        self.patch_embed_row = Signal_PatchEmbed(
            img_size=(img_h, img_w),
            patch_sizew=row_w,
            patch_sizeh= row_h,
            in_chans=in_chans,
            embed_dim=embed_dim_h[0],
            use_conv_embed=use_conv_embed,
            norm_layer=norm_layer if self.patch_norm else None,
            is_stem=True)
        self.patch_embed_col = Signal_PatchEmbed(
            img_size=(img_h, img_w),
            patch_sizew=col_w,
            patch_sizeh=col_h,
            in_chans=in_chans,
            embed_dim=embed_dim_v[0],
            use_conv_embed=use_conv_embed,
            norm_layer=norm_layer if self.patch_norm else None,
            is_stem=True)

        #num_patches = self.patch_embed.num_patches
        patches_resolution_row = self.patch_embed_row.patches_resolution
        self.patches_resolution_row = patches_resolution_row
        patches_resolution_col = self.patch_embed_col.patches_resolution
        self.patches_resolution_col = patches_resolution_col
        self.pos_drop = nn.Dropout(p=drop_rate)

        # stochastic depth
        dpr_h = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths_h))]  # stochastic depth decay rule
        dpr_v = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths_v))]  # stochastic depth decay rule

        # build layers Horziontal
        self.layers = nn.ModuleList()
        for i_layer in range(self.num_layers_h):
            layer = BasicLayer(dim=embed_dim_h[i_layer],
                               out_dim=embed_dim_h[i_layer + 1] if (i_layer < self.num_layers_h - 1) else None,
                               input_resolution=(patches_resolution_row[0] // (2 ** i_layer),
                                                 patches_resolution_row[1] // (2 ** i_layer)),
                               depth=depths_h[i_layer],
                               mlp_ratio=self.mlp_ratio,
                               drop=drop_rate,
                               drop_path=dpr_h[sum(depths_h[:i_layer]):sum(depths_h[:i_layer + 1])],
                               norm_layer=norm_layer,
                               downsample=PatchEmbed if (i_layer < self.num_layers_h - 1) else None,
                               focal_level=focal_levels_h[i_layer],
                               focal_window=focal_windows_h[i_layer],
                               use_conv_embed=use_conv_embed,
        ...
        for i_layer in range(self.num_layers_v):
            layer = BasicLayer(dim=embed_dim_v[i_layer],
                               out_dim=embed_dim_v[i_layer + 1] if (i_layer < self.num_layers_v - 1) else None,
                               input_resolution=(patches_resolution_col[0] // (2 ** i_layer),
                                                 patches_resolution_col[1] // (2 ** i_layer)),
                               depth=depths_v[i_layer],
                               mlp_ratio=self.mlp_ratio,
                               drop=drop_rate,
                               drop_path=dpr_v[sum(depths_v[:i_layer]):sum(depths_v[:i_layer + 1])],
                               norm_layer=norm_layer,
                               downsample=PatchEmbed if (i_layer < self.num_layers_v - 1) else None,
                               focal_level=focal_levels_v[i_layer],
                               focal_window=focal_windows_v[i_layer],
                               use_conv_embed=use_conv_embed,
                               use_checkpoint=use_checkpoint,
                               use_layerscale=use_layerscale,
                               layerscale_value=layerscale_value,
        ...
        self.norm_h = norm_layer(self.num_features_h)
        self.norm_v = norm_layer(self.num_features_v)
        self.avgpool = nn.AdaptiveAvgPool1d(1)
        self.head = nn.Linear(self.num_features_v+self.num_features_h, num_classes) if num_classes > 0 else nn.Identity()
        self.head_h = nn.Linear(self.num_features_h, num_classes) if num_classes > 0 else nn.Identity()
        self.head_v = nn.Linear(self.num_features_v, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def forward_features_rows(self, x):
        x, H, W = self.patch_embed_row(x)
        x = self.pos_drop(x)

        for layer in self.layers:
            x, H, W = layer(x, H, W)
        x = self.norm_h(x)  # B L C
        x = self.avgpool(x.transpose(1, 2))  # B C 1
        x = torch.flatten(x, 1)
        return x
    def forward_features_cols(self, x):
        x, H, W = self.patch_embed_col(x)
        x = self.pos_drop(x)

        for layer in self.layers1:
            x, H, W = layer(x, H, W)
        x = self.norm_v(x)  # B L C
        x = self.avgpool(x.transpose(1, 2))  # B C 1
        x = torch.flatten(x, 1)
        return x

    def forward(self, x):
        if self.col_or_row == 'all':
            x1 = self.forward_features_rows(x)
            x2 = self.forward_features_cols(x)
            x3 = torch.cat((x1,x2), dim=1)
            x3 = self.head(x3)
        elif self.col_or_row == 'col':
            x2 = self.forward_features_cols(x)
            x3 = self.head_v(x2)
        elif self.col_or_row == 'row':
            x2 = self.forward_features_rows(x)
            x3 = self.head_h(x2)
        return x3
...
if __name__ == '__main__':
    ...
    x = torch.rand(16, 1, 12, 2000)
    #model = FocalNet(depths=[2, 2, 6, 2], embed_dim=96, focal_levels=[3, 3, 3, 3])
    # col means vertical
    # row means horzontal
    model = Signals_FocalNet(depths_h=[2, 6,2],depths_v=[2, 6], embed_dim=96, focal_levels_v=[3, 3],focal_levels_h=[3, 3, 3], row_h=3, row_w=100, col_h=6, col_w=3, img_h=12, img_w=2500, in_chans=1, num_classes=2, col_or_row='row')
