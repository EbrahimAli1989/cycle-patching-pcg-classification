...
# --------------------------------------------------------
#
# Unchanged from the original config.py (only relocated into the package).
# `get_config(args)` returns a `yacs.config.CfgNode` -- already a proper
# object with `.defrost()`/`.freeze()`/`.dump()` etc. -- so it is used as-is
# rather than wrapped in another class. See README.md > "Repository notes".
...
def update_config(config, args):
    ...
    config.OUTPUT = os.path.join(config.OUTPUT, args.modelname, config.TAG)
